...
export class Rhino {
   ...
   * This will overwrite anything that in saved in this path. RhinoContent options must have
   * 'forceWrite: false' or be empty to use the trained model correctly.
  ...
  public static async trainContextFromDynamicSlots(
    ...
    if (slots) {
      if (typeof slots !== 'object') {
        throw new Error(
          'Slots must be a dictionary mapping slot names to sequences of string value'
        );
      }
      ...
      if (
        !content?.context?.slots ||
        typeof content.context.slots !== 'object'
      ) {
        throw new Error('Invalid value in slots field');
      }

      const merged: Record<string, string[]> = {};

      for (const [k, v] of Object.entries(content.context.slots)) {
        if (!(k in slots)) {
      ...
      for (const [key, values] of Object.entries(slots)) {
        ...
        if (!(key in content.context.slots)) {
          throw new Error(`Slot '${key}' does not exist`);
        }
      ...
      for (const [key, values] of Object.entries(merged)) {
        ...
        for (const v of values) {
          ...
          if (seen.has(value)) {
            throw new Error(`Duplicate slot value '${v}' in '${key}'`);
          }
          seen.add(value);
        }
      }
      ...
      yamlContent = stringify(content);
    }

    await this.trainContextFromYaml(
      accessKey,
      writePath,
      language,
      yamlContent
  ...
  public static async trainContextFromYaml(
    ...
    const res = await fetch(`${PV_API_URL}${language}/api/rhn`, {
      method: 'POST',
      headers: {
    ...
    this._processMutex
      .runExclusive(async () => {
        ...
        if (isFinalized === 1) {
          ...
          if (isUnderstood === -1) {
            throw new RhinoErrors.RhinoInvalidStateError(
              'Rhino failed to process the command'
            );
          }
          ...
          if (isUnderstood === 1) {
            ...
            const intentAddress =
              this._module.HEAP32[
                this._intentAddressAddress / Int32Array.BYTES_PER_ELEMENT
              ];
            ...
            const numSlots =
              this._module.HEAP32[
                this._numSlotsAddress / Int32Array.BYTES_PER_ELEMENT
              ];
            if (numSlots === -1) {
              throw new RhinoErrors.RhinoInvalidStateError(
                'Rhino failed to get the number of slots'
              );
            }
            ...
            for (let i = 0; i < numSlots; i++) {
              ...
              if (!slot) {
                throw new RhinoErrors.RhinoInvalidStateError(
                  'Rhino failed to get the slot'
                );
              }
              const value = this._getSlotValue(i);
              if (!value) {
                throw new RhinoErrors.RhinoInvalidStateError(
                  'Rhino failed to get the slot value'
                );
              }
              slots[slot] = value;
            }

            const slotsAddressAddress =
              this._module.HEAP32[
                this._slotsAddressAddressAddress / Int32Array.BYTES_PER_ELEMENT
              ];
            const valuesAddressAddress =
              this._module.HEAP32[
                this._valuesAddressAddressAddress / Int32Array.BYTES_PER_ELEMENT
              ];
  ...
  private _getSlot(index: number): string {
    ...
    const slotAddress =
      this._module.HEAP32[
        slotsAddressAddress / Int32Array.BYTES_PER_ELEMENT + index
      ];
  ...
  private _getSlotValue(index: number): string {
    ...
    const valueAddress =
      this._module.HEAP32[
        valuesAddressAddress / Int32Array.BYTES_PER_ELEMENT + index
      ];
  ...
  private static async initWasm(
    ...
    if (sensitivity && !(typeof sensitivity === 'number')) {
      ...
      throw new RhinoErrors.RhinoInvalidArgumentError(
        'Rhino sensitivity is outside of range [0, 1]'
      );
    }
    ...
    const contextPathAddress = module._malloc(
    ...
    const contextInfoAddressAddress = module._malloc(
      Int32Array.BYTES_PER_ELEMENT
    );
    ...
    const contextInfoAddress =
      module.HEAP32[contextInfoAddressAddress / Int32Array.BYTES_PER_ELEMENT];
    ...
    const slotsAddressAddressAddress = module._malloc(
      Int32Array.BYTES_PER_ELEMENT
    );
    ...
    const valuesAddressAddressAddress = module._malloc(
      Int32Array.BYTES_PER_ELEMENT
    );
  ...
  protected static wrapAsyncFunction(
    module: RhinoModule,
    functionName: string,
    numArgs: number
  ): (...args: any[]) => any {
