...
export class Rhino {
  ...
  /**
   * Trains a model using a Rhino context (.rhn file), overriding slot values.
   *
   * @param accessKey AccessKey generated by Picovoice Console.
   * @param writePath Absolute path where the trained model will be saved.
   ...
   * @param rhinoContext RhinoContext object containing a base64 representation of or path to a public binary Rhino context model.
   * @param model RhinoModel configuration to be used for training.
   * @param slots Object mapping existing slot names to additional slot values to merge into the YAML's
  ...
  public static async trainContextFromDynamicSlots(
    ...
    if (slots) {
      if (typeof slots !== "object") {
        throw new Error("Slots must be a dictionary mapping slot names to sequences of string value");
      }
      ...
      if (!content?.context?.slots || typeof content.context.slots !== "object") {
      ...
      for (const [key, values] of Object.entries(slots)) {
        if (!(values instanceof Set)) {
          throw new Error(`Slot '${key}' must be a non-empty set of string values`);
        }

        if (!(key in content.context.slots)) {
          throw new Error(`Slot '${key}' (${JSON.stringify(content.context.slots)}) does not exist`);
        }
    ...
    await this.trainContextFromYaml(accessKey, writePath, language, yamlContent);
  }
  ...
  public static async trainContextFromYaml(
    ...
    if (!VALID_LANGUAGES.includes(language)) {
      throw new RhinoErrors.RhinoInvalidArgumentError(`Invalid language ('${language}')`);
    }
    ...
    if (!res.ok) {
      ...
      throw new RhinoErrors.RhinoRuntimeError(`Failed to train model: ${errorBody}`);
    }
