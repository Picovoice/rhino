...
export class Rhino {
   ...
   * @param language Two-letter language code ('de', 'en', 'es', 'fr', 'it', 'ja', 'ko', 'pt').
   ...
   * @param slots Key-value pairs to override context slot values.
   */
  public static async trainWithContext(
    accessKey: string,
    writePath: string,
    language: string,
    ...
    const context = rhino.contextInfo;
    await rhino.release();
    await this.trainWithYaml(accessKey, writePath, language, context, slots);
  }
   ...
   * @param writePath Absolute path where the trained model will be saved.
   * @param language Two-character language code ('de', 'en', 'es', 'fr', 'it', 'ja', 'ko', 'pt').
   * @param context YAML configuration in string to be used for training.
   * @param slots Additional slot key-value pairs merged into the YAML's `context.slots` section.
   */
  public static async trainWithYaml(
    accessKey: string,
    writePath: string,
    language: string,
    context: string,
    ...
    let yamlContext = context;
    if (slots) {
      const content = parse(yamlContext);
      ...
      yamlContext = stringify(content);
    }

    const payload = {
      platform: "wasm",
      yaml_content: yamlContext,
    };
