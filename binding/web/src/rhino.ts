...
export class Rhino {
   ...
   * @param contextPath The path to the context file saved in indexedDB.
   * @param modelPath Path to the model saved in indexedDB.
   * @param initConfig Optional configuration arguments.
   *
   * @returns An instance of the Rhino engine.
   */
   ...
   * @return The Rhino inference (`isFinalized` will always be present: when it's true the entire object will be populated)
   */
  public async process(pcm: Int16Array): Promise<RhinoInference> {
    ...
    return new Promise<RhinoInference>((resolve, reject) => {
      this._processMutex
        .runExclusive(async () => {
