...
export function getAudioFileByLanguage(
  ...
  return path.join(ROOT_DIR, 'resources/audio_samples', audioFileName);
}

export function getWithinContextParameters(): [
  string,
  string,
  string,
  ...
  return withinContextJson.map((x: any) => [
    x.language,
    x.context_name,
    x.inference.intent,
    x.inference.slots,
...
export function getOutOfContextParameters(): [string, string][] {
  ...
  return outOfContextJson.map((x: any) => [x.language, x.context_name]);
}
